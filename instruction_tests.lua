...
describe("the uxn instruction", function()
	...
	pending("SWAP", function() end)

	pending("OVER", function() end)
	...
	pending("GTH", function() end)

	pending("LTH", function() end)
	...
	pending("JCN", function() end)

	pending("JSR", function() end)
	...
	describe("DEI", function()
		before_each(function()
			...
			func_device:addPort(2, true, function()
				return 0x87a9
			end, nil)
			func_device:addPort(4, true, function()
				return 0x34
			end, nil)

			func_device:addPort(6, false, function()
				return 0xab
			end, nil)
			func_device:addPort(7, false, function()
				return 0x1337
			end, nil)
		...
		it("fetches stored bytes", function()
			run_program({
				0x80,
				0x17, -- Address of too long byte
				0x16, -- DEI

				0x80,
				0x12, -- Address of short port
				0x16, -- DEI

				0x80,
				0x16, -- Address of proper byte port
				0x16, -- DEI
			})
		...
		it("fetches stored shorts", function()
			run_program({
				...
				0x80,
				0x14, -- Address of too short short port
				0x36, -- DEI2

				0x80,
				0x16, -- Address of byte port
				0x36, -- DEI2
			})
		...
		pending("fetches function bytes", function() end)
	end)
end)
